...
- "Chapter X – ..." (copy the full title after the dash)
- "Part X – ..." or "Section X – ..."
...
{{
  ...
  "toc": {{
    "has_toc": true,
    "entries": [
      {{"level": 1, "title": "Chapter 1 – Page 1 of the T2 return", "page": 24, "source": "extracted"}},
      {{"level": 1, "title": "Chapter 2 – Page 2 of the T2 return", "page": 31, "source": "extracted"}},
      {{"level": 2, "title": "Line 001 – Business Number", "page": 39, "source": "extracted"}}
    ]
  }},
  "content_regions": [
    {{
...
def classify_with_llm(content: str, title: str = "", provider_name: str = "glm-api") -> tuple[dict, float]:
    """Use LLM provider to comprehensively analyze document.

    Args:
        content: Document content to analyze
        title: Document title
        provider_name: LLM provider name (glm-api, claude, gemini, etc.)
    ...
    if not provider or not provider.is_available():
        raise ValueError(f"Provider '{provider_name}' not available. Please check configuration.")
    ...
    try:
        # Call LLM provider
        logger.info(f"Calling {provider_name} for comprehensive analysis...")
        ...
        logger.error(f"LLM analysis failed ({provider_name}): {e}")
        processing_time = time.time() - start_time
...
def classify_content(
    ...
    provider: str = "glm-api"
) -> dict:
    """
    Classify extracted content using LLM semantic analysis.
    ...
    Args:
        ...
        provider: LLM provider name (glm-api, claude, gemini, etc.)
    ...
    print(f"Method: {provider} (Dynamic Classification)")
    ...
    print(f"\n🧠 Analyzing document with {provider}...")
    print(f"   This will take ~{len(total_text) // 1000 * 1 // 60} minutes for {len(total_text):,} chars")
    print(f"   Using {provider} for dynamic document classification...")

    try:
        pdf_title = extraction_data.get('pdf_path', '').split('/')[-1]

        # Perform comprehensive document analysis using LLM provider
        analysis, processing_time = classify_with_llm(
            content=total_text,
            title=pdf_title,
        ...
        classification_data = {
            ...
            "reasoning": classification.get("reasoning", ""),
            "semantic_tags": semantic_tags,
            "method": f"{provider}_comprehensive",
            "model": provider,
        ...
        logger.error(f"LLM classification failed ({provider}): {e}")
        raise
    ...
    print(f"\n🗺️ Content Regions: {len(regions)} identified")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Classification with GLM API (default)
  python stage2_classify_content.py --extraction-id abc123 --provider glm-api

  # Classification with other providers
  python stage2_classify_content.py --extraction-id abc123 --provider claude
  python stage2_classify_content.py --extraction-id abc123 --provider gemini

  # Force re-classification (ignore cache)
  python stage2_classify_content.py --extraction-id abc123 --provider glm-api --force
  ...
  python -c "from app.document_processor.pipeline_manager import CacheManager; \\
    ...
    parser.add_argument(
        ...
        help='LLM provider to use (default: glm-api)'
    )
